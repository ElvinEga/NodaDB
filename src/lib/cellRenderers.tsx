...
export function BooleanCell({ value }: { value: any }) {
  const isTrue = value === true || value === 1 || value === 'true' || value === 't';
  const isFalse = value === false || value === 0 || value === 'false' || value === 'f';
 ...
 */
export function NumberCell({ value, dataType }: { value: any; dataType: string }) {
 ...
 */
export function TextCell({ value, maxLength = 50 }: { value: any; maxLength?: number }) {
  if (value === null || value === undefined) {
    return <span className="text-muted-foreground italic">NULL</span>;
  }
